...
export const register = (
  ...
  app.get("/apim-delegation", async (req, res) => {
    ...
    switch (operation) {
      ...
      case "Subscribe":
        ...
        const product = await apimService.getProduct(
          subscribeRequest.productId
        );
      ...
      case "Unsubscribe":
        ...
        await apimService.updateSubscriptionState(
  ...
  app.post("/subscribe", async (req, res) => {
    ...
    const product = await apimService.getProduct(req.body.productId as string);
    let amount: number = 0;

    if (product) {
      const monetizationModel =
        await MonetizationService.getMonetizationModelFromProduct(product);

      if (
        monetizationModel &&
        (monetizationModel.pricingModelType === "Tier" ||
          monetizationModel.pricingModelType === "TierWithOverage" ||
          monetizationModel.pricingModelType === "Unit")
      ) {
  ...
  app.get("/checkout", async (req, res) => {
    ...
    res.render(`checkout-${process.env.PAYMENT_PROVIDER.toLowerCase()}`, {
      subscribeRequest,
      subscriptionName,
      userEmail,
      title: "Checkout",
  ...
  app.post("/signIn", async (req, res) => {
    ...
    const { authenticated, userId } = await ApimService.authenticateUser(
      email,
      password
    );
    ...
    const redirectUrl =
      process.env.APIM_DEVELOPER_PORTAL_URL + "/signin-sso?" + redirectQuery;
...
function validateSignUpSignInRequest(
  signUpSignInRequest: SignUpSignInRequest
): boolean {
  return validateRequest(
    [signUpSignInRequest.salt, signUpSignInRequest.returnUrl],
    signUpSignInRequest.sig
  );
}
...
function validateUnsubscribeRequest(
  subscriptionRequest: SubscriptionRequest
): boolean {
  return validateRequest(
    [subscriptionRequest.salt, subscriptionRequest.subscriptionId],
    subscriptionRequest.sig
  );
}
