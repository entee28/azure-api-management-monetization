...
export class ApimService {
  ...
  resourceGroupName: string =
    process.env.APIM_SERVICE_AZURE_RESOURCE_GROUP_NAME;
  ...
  public async getProducts(): Promise<ProductListByServiceResponse> {
    ...
    return await this.managementClient.product.listByService(
  ...
  public async getSubscriptions(): Promise<SubscriptionListResponse> {
    ...
    return await this.managementClient.subscription.list(
      this.resourceGroupName,
      this.serviceName
    );
  }
  ...
  public async getProduct(productId: string): Promise<ProductGetResponse> {
    try {
      ...
      return await this.managementClient.product.get(
        this.resourceGroupName,
        this.serviceName,
        productId
      );
      ...
      console.log("ERROR PRODUCTID", productId);
    }
  }
  ...
  public async getUser(userId: string): Promise<UserGetResponse> {
    ...
    return await this.managementClient.user.get(
      this.resourceGroupName,
      this.serviceName,
      userId
    );
  ...
  public async getSubscription(sid: string): Promise<SubscriptionGetResponse> {
    ...
    return await this.managementClient.subscription.get(
      this.resourceGroupName,
      this.serviceName,
      sid
    );
  ...
  public async getUsage(
    ...
    to: Date,
    from?: Date
  ): Promise<ReportRecordContract> {
    await this.initialize();
    const filter = `timestamp ge datetime'${
      from ? from.toISOString() : new Date("2000-01-01").toISOString()
    }' and timestamp le datetime'${to.toISOString()}' and subscriptionId eq '${sid}'`;
    const report = await this.managementClient.reports.listBySubscription(
      this.resourceGroupName,
      this.serviceName,
      filter
    );
  ...
  public async getUserSubscriptions(
    ...
    return await this.managementClient.userSubscription.list(
      this.resourceGroupName,
      this.serviceName,
      userId
    );
  ...
  public async updateSubscriptionState(sid: string, state: SubscriptionState) {
    ...
    return await this.managementClient.subscription.update(
      this.resourceGroupName,
      this.serviceName,
      sid,
      { state },
  ...
  public async getSharedAccessToken(
    userId: string
    ...
    return await this.managementClient.user.getSharedAccessToken(
      this.resourceGroupName,
      this.serviceName,
      userId,
      { expiry, keyType: "primary" }
    );
  }
  ...
  public static async authenticateUser(
    ...
    try {
      ...
      const managementApiUrl = Utils.ensureUrlArmified(
        process.env.APIM_MANAGEMENT_URL
      );
