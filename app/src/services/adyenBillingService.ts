    ...
    await Promise.all(
      subscriptions
        ...
        .map(async (sub) => {
          ...
          const usageReport = await apimService.getUsage(
            sub.name,
            endDate,
            startDate
          );
          ...
          const product = await apimService.getProduct(
            sub.scope.split("/").slice(-1)[0]
          );

          if (product) {
            const monetizationModel =
              await MonetizationService.getMonetizationModelFromProduct(
                product
              );
              ...
              if (usageReport) {
                ...
                switch (monetizationModel.pricingModelType) {
                  ...
                  case "TierWithOverage":
                    ...
                    let usageOverage = Math.floor(
                      usageUnits - monetizationModel.prices.unit.quota
                    );
                    ...
                    amount =
                      monetizationModel.prices.unit.unitAmount +
                      usageOverage *
                        monetizationModel.prices.metered.unitAmount;
                  ...
                  case "Unit":
                    ...
                    amount =
                      numberOfUnits * monetizationModel.prices.unit.unitAmount;
                    currency = monetizationModel.prices.unit.currency;
                    break;
                  default:
                    break;
  ...
  public static async takePaymentFromUser(invoice: Invoice) {
    ...
    if (invoice.amount > 0) {
      ...
      if (paymentMethodsResponse.storedPaymentMethods) {
        ...
        const paymentResponse = await checkout.payments({
          ...
          paymentMethod: {
            ...
            storedPaymentMethodId:
              paymentMethodsResponse.storedPaymentMethods[0].id,
          },
        ...
        if (
          paymentResponse.resultCode === "Error" ||
          paymentResponse.resultCode === "Cancelled" ||
          paymentResponse.resultCode === "Refused"
        ) {
        ...
        console.log("The else logic ran!");
